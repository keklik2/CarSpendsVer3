...
class GetNoteItemsListUseCase @Inject constructor(private val repository: NoteRepository) {
    suspend operator fun invoke(date: Long = ALL_TIME) = repository.getNoteItemsListUseCase(date)
    suspend operator fun invoke(type: NoteType?, date: Long = ALL_TIME) = repository.getNoteItemsListUseCase(type, date)
}
